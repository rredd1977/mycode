...
terraform {
  required_providers {   # without the required providers inside of the module, the root module may have problems when calling the module
    docker = {
      ...
      # version = "4.0.0"   # Do not use ~> (or other maximum-version constraints) for modules you intend to reuse across many configurations...  Specify a minimum version.. let the root module manage the maximum version.
    }
  }
}
